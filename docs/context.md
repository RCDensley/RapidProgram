...
| 2026-05-11 | #3 | Dashboard only had `data` from `useApp()` — needed to add `setData` to the destructure. Null-narrowing inside an IIFE doesn't flow through TS narrowing from the outer `&&` guard — reassigned `currentPhase` to a local `phase_` const inside the IIFE to make the type explicit. Criteria block rendered only when `currentPhase` exists AND `criteria.length > 0` — no empty state shown. |
| 2026-05-11 | #4 | Criteria injected inline into the existing phase string in `buildSystemPrompt` — no new prompt section needed. Format: `Deliver: 01 Jun → 31 Aug [criteria: ✓ Done item; ○ Pending item]`. Phases with no criteria omit the bracket entirely. Types in the API are `any` (appdata.json is parsed as untyped JSON), so used an inline cast `as { text: string; done: boolean }[]` to satisfy TS without importing frontend types into the API. |
| 2026-05-11 | #5 | `ChatMessage`, `ThreadType`, `ChatThread` added before the Reporting section in types/index.ts. `threads: ChatThread[]` added to `AppData`, `defaultData`, and the storage merge — same pattern as `reports`. No migration needed; existing blobs without the field load as `[]`. |
| 2026-05-11 | #6 | Removed local `ChatMessage` interface from Assistant.tsx (now imported from types). `messages` state replaced by `activeThreadId` + derived `displayMessages = activeThread?.messages ?? []`. Streaming uses local `streamingContent: string\|null` state — accumulates deltas, renders as a live message, commits to thread on completion (avoids N blob writes during streaming). File repo moved to a right-side toggle panel (FileStack icon in header). `ThreadRow` and `ThreadGroup` defined as inner functions inside `Assistant` to close over `activeThreadId` and `pinThread`. `buildNewThread()` returns both the new thread and the full updated data object so `sendMessage` can do a single `setData` call combining new thread + user message. |
| 2026-05-11 | #7 | Implemented as part of #6 — no separate commit needed. All test criteria were met by the Issue 6 implementation. |
| 2026-05-11 | #8 | `autoTitleThread` is fire-and-forget (not awaited). Uses `latestDataRef` (a `useRef` kept current via `useEffect`) to access fresh data when committing the title — avoids stale closure issue since the AI call resolves after the streaming finally block has already updated `data`. Title prompt: `"Reply with ONLY a 4-6 word thread title (no punctuation, no quotes)"`. Silently fails on any error — thread keeps 'New Thread' title. Only fires when `isNewThread` is true (captured before the thread-creation branch in sendMessage). |
| 2026-05-11 | #9 | `ActivityEntry` and `ActivityBatch` types in `api/src/types/activity.ts`. `validateDaemonKey` checks `X-Daemon-Key` header against `DAEMON_API_KEY` env var — returns false if either side is empty (misconfigured treated as invalid). `DAEMON_API_KEY: "localdevkey"` added to `local.settings.json`. |
| 2026-05-11 | #10 | POST /api/activity implemented in same file as GET — reads existing daily blob, appends the new `ActivityBatch`, writes back. `blobClient.upload()` does not accept `overwrite` option in this SDK version — omitted (upload to existing blob overwrites by default). |
| 2026-05-11 | #11 | GET /api/activity accepts daemon key OR anonymous browser access (checks header presence before validating). `buildSystemPrompt` takes optional `activityBatches?: any[]` second parameter. `buildActivitySection` formats each entry by type: `app_focus` → timestamp + app name + duration; `audio_transcript` → transcript text; `screen_context` → AI description. Handler reads `activity-log/YYYY-MM-DD.json` before building the prompt — silently omits the section if blob is empty or missing. |
| 2026-05-11 | #12 | `setuptools.backends.legacy:build` is not valid in older pip — correct backend is `setuptools.build_meta`. `tomllib` is stdlib in Python 3.11+ so no dependency needed. `config.toml` is gitignored; `config.example.toml` committed with the real `api_url` pre-filled. `REQUIRED_KEYS` validation exits with a clear message before the scheduler loop. Bad `api_url` (doesn't start with `http`) logs a warning but does not exit. `capture/__init__.py` created ready for Issues 13/17/18. |
| 2026-05-11 | #13 | `flush()` must close the in-progress window (not just completed ones) — otherwise staying on one window for a full 15-min batch produces zero entries. Fix: set `last_seen = time.monotonic()`, append to completed, reset `_current = None` at flush time. App name extraction uses ` - ` split (last segment) as best-effort — window title is the reliable field. `NOISE_WINDOWS` set filters Task Switching / lock screen by title. |
| 2026-05-11 | #14 | `uploader.py` separates concerns cleanly: `upload_batch()` calls `_post()` then `_drain_queue()`. Retry queue is `pending-batches.jsonl` (one JSON object per line). Batches older than 24h are pruned on drain. 401 from API logs a distinct error message (key misconfiguration). daemon.py wires `do_sample()` on `POLL_INTERVAL` seconds and `do_flush()` on `batch_interval_minutes`. Final flush on KeyboardInterrupt ensures no data is lost on clean shutdown. |
| 2026-05-11 | #15 | `/api/checkin` uses `ai.complete()` (non-streaming) since the result is saved to blob, not streamed to a browser. Builds thread server-side with `type: 'checkin'`, saves to `appdata.json`. Thread is `unshift`-ed to keep newest first. `checkinTitle()` uses local server time (`new Date()`) — matches AEST on a correctly configured Azure region. |
| 2026-05-11 | fix | Daemon timestamps changed from UTC (`datetime.now(timezone.utc)`) to local time (`datetime.now().isoformat(timespec='seconds')`) in both `capture/apps.py` and `uploader.py`. AI prompt timestamp slices `[11:16]` will now show AEST times matching the user's clock. Production daemon connects to `http://localhost:7071` for local dev — production URL blocked by SWA Easy Auth (OQ3, deferred). |
| 2026-05-11 | #16 | `winotify` already installed globally on user's machine (version 1.1.0). VS Code shows a hint about it not being in the project venv — not actionable, daemon runs fine. `notify_checkin()` in `notifier.py` uses `Notification.launch` to open the assistant URL on click. `do_checkin()` in daemon.py has a 60s timeout (AI call takes a moment). Fails silently on network errors — logs warning, retries next interval. |
| 2026-05-11 | #17 | `torch.hub.load()` requires `trust_repo=True` to bypass interactive confirmation prompt (non-interactive subprocess would hang/fail without it). `packaging` package missing from torch's transitive deps — add explicitly. whisper-small (461MB) downloaded on first run, cached locally after. AudioTracker runs in a daemon thread; `start()` returns immediately, models load in background. `_transcribe_buffer()` takes the lock briefly to swap the buffer then does the slow Whisper call outside the lock. |
| 2026-05-11 | #18 | Screen capture uses mss (primary monitor index 1, not 0 which is the virtual full-desktop). Image resized to 50% JPEG quality 60 before encoding — ~110KB payload. Sends to /api/ai as a multipart vision message (same SSE stream as chat). `_pending_screen` list lives in `do_flush` closure scope — avoids a class and keeps the screen capture stateless. Screen locked at capture time produces no entry (mss raises or returns black frame; handled by outer try/except). |
