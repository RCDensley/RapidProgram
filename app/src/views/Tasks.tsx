      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5, flexWrap: 'wrap' }}>
            ...
            {/* Status badge — only show when not Open */}
            {status !== 'Open' && (
...
// ─── Bucket column ────────────────────────────────────────────────────────────
function BucketColumn({ bucket, tasks, sows, sowId, onOpenTask, onCompleteTask, onDragStart, onDrop, onAddTask }: {
  ...
  return (
    <div
      ...
      style={{ flex: 1, minWidth: 0, background: isDragOver ? 'rgba(56,189,248,0.05)' : 'var(--surface)', border: `1px solid ${isDragOver ? 'var(--sky)' : 'var(--border)'}`, borderRadius: 'var(--radius)', padding: '10px 10px 6px', transition: 'all 0.15s', minHeight: 80 }}
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{BUCKET_LABELS[bucket]}</span>
          {incomplete.length > 0 && <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', background: 'var(--border)', color: 'var(--text-2)', padding: '1px 6px', borderRadius: 10 }}>{incomplete.length}</span>}
        </div>
        <button onClick={() => onAddTask(bucket, sowId)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 2, borderRadius: 4 }}
      ...
      {completed.length > 0 && <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 6, textAlign: 'center', fontWeight: 600 }}>{completed.length} completed</div>}
    </div>
  )
}
...
function TaskPanel({ task, sows, projectFiles, onClose, onSave, onDelete, onAddAttachment }: {
  ...
  // Status change — Done syncs with completedAt
    ...
    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 440, background: 'var(--surface)', borderLeft: '1px solid var(--border)', zIndex: 50, display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,0.4)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '18px 20px' }}>
        ...
        <div className="field">
          ...
          <textarea className="field-input" rows={3} style={{ resize: 'vertical', fontFamily: 'var(--font-main)' }}
            value={t.description ?? ''} onChange={e => setT(p => ({ ...p, description: e.target.value }))} />
        </div>
        ...
        <div className="field-row">
          <div className="field">
            ...
            <select className="field-input" value={t.status ?? 'Open'}
              onChange={e => handleStatusChange(e.target.value as TaskStatus)}
              style={{ color: STATUS_COLORS[t.status ?? 'Open'], fontWeight: 700 }}>
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <input className="field-input font-mono" type="number" min={0.5} step={0.5} style={{ width: 72 }}
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <select className="field-input" value={t.recurrence?.type ?? '__none__'}
              onChange={e => {
            ...
            {t.recurrence && (
              <input className="field-input font-mono" type="number" min={1} max={12} style={{ width: 64 }} title="Interval"
        ...
        <div className="field">
          ...
          {attachedFiles.map(f => (
            <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, padding: '6px 8px', background: 'var(--card)', borderRadius: 'var(--radius-sm)' }}>
              ...
              <button className="icon-btn" style={{ color: 'var(--text-3)', opacity: 0.5, flexShrink: 0 }} onClick={() => removeAttachment(f.id)}
          ...
          <button className="btn-ghost btn-sm" style={{ width: '100%', justifyContent: 'center' }}
            ...
            disabled={uploading}>
            {uploading ? <><Loader2 size={11} className="animate-spin" /> Uploading and classifying...</> : <><Paperclip size={11} /> Attach file</>}
          </button>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 4 }}>
            Files are also added to the file repository and AI-classified using the task context.
          </div>
        </div>
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <input className="field-input field-input-sm" placeholder="Add a comment..." value={newComment} style={{ flex: 1 }}
...
export default function Tasks() {
  ...
  // Upload a file, passing task context to improve AI classification
