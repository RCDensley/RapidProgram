...
import {
  ...
  AlertTriangle, Info, X, Users, Calendar, ChevronRight,
} from 'lucide-react'
...
// ─── Burndown chart ───────────────────────────────────────────────────────────
function BurndownChart({ sowName, data: chartData }: { sowName: string; data: any[] }) {
  return (
    <div>
      <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 16, fontWeight: 600 }}>
        Forecast <span style={{ color: 'var(--violet-bright)' }}>●</span> vs
        Actual <span style={{ color: 'var(--sky-bright)' }}>●</span> — every other week shown
      </div>
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" />
          <XAxis dataKey="week" tick={{ fill: '#64748b', fontSize: 10, fontFamily: 'JetBrains Mono' }} />
          <YAxis tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} tick={{ fill: '#64748b', fontSize: 10, fontFamily: 'JetBrains Mono' }} width={44} />
          <Tooltip
...
export default function Dashboard() {
  ...
  // Burndown data — SOW-specific when selected, first SOW when not
  const chartSow      = selectedSow ?? data.sows[0] ?? null
          ...
          {selectedSow ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '18px 20px' }}>
                ...
                <BurndownChart sowName={selectedSow.shortName} data={burndownData} />
              </div>
            </div>
            ...
            <div>
              <h2 className="section-title">
                Program Burndown — {chartSow?.shortName ?? '—'}
                <span style={{ fontSize: 10, color: 'var(--text-3)', marginLeft: 8, fontWeight: 600, textTransform: 'none', letterSpacing: 0 }}>
                  select a SOW to inspect
                </span>
              </h2>
              <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '18px 20px', marginBottom: 20 }}>
                <BurndownChart sowName={chartSow?.shortName ?? ''} data={burndownData} />
                {/* SOW tabs */}
                  ...
                  {data.sows.map(sow => (
                    <button
                      ...
                      style={{
                        ...
                        background: chartSow?.id === sow.id ? sow.color + '22' : 'transparent',
